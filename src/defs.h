...
extern void render_turn(SDL_Renderer *renderer, Game *partie);
   ...
   //fonctions gérant le fichier de sauvegarde
//Permet de suprimmer une partie terminée du fichier de sauvegarde
extern void supprimerPartie(int id);
//Permet de mettre à jour une partie du fichier de sauvegarde
extern void Save( Game * game );
//Permet de créer une partie dans le fichier de sauvegarde
extern void nouvellePartie(int ia, Game * partie ,int idJB,int idJN);
//Permet de charger une partie du fichier de sauvegarde
extern void chargerPartie(int id, Game * game);
//Permet de placer tout le contenu du fichier de sauvegarde des parties dans un tableau
extern int listerParties(Game parties[]);
//Renvoie le nombre de parties enregistrées
extern int saveSize();
//Permet de créer un joueur dans le fichier de sauvegarde
extern void nouveauJoueur(char nom[]);
//Permet de charger un joueur du fichier de sauvegarde
extern void chargerJoueur(int id, Joueur * joueur);
//Permet de mettre à jour un joueur du fichier de sauvegarde
extern void SaveJoueur( Joueur * joueur );
//Renvoie le nombre de joueurs enregistrés
extern int nbJoueur();
//Permet de placer tout le contenu du fichier de sauvegarde des joueurs dans un tableau
...
//Fonctions de l'IA
//fonction qui renvoie la valeur de la case du tableau plt, mais en prévoyant un code d'erreur pour les requettes sortants du tableau
extern int arrAcc(int plt[][8],int x, int y);
//fonction qui calcule le gain relatif d'effectuer un déplacement
extern float poids(int color,Square plt[][8],int frmX,int frmY,int toX,int toY,JoueurIA * jN);
//fonction qui établit la liste des action possibles de l'ia
extern int listActions(Possibillite actionlist[] , Square plateau[][8], JoueurIA * jN);
//fonction qui sélectionne la meilleure action que l'IA peut effectuer
extern int findBestAction(Possibillite *bestAction , Square plateau[][8],JoueurIA * jN);
//fonction de deplacement conçue pour l'ia et ses calculs
extern void deplacement(Possibillite dpl, Square plat[][8],int color);
//fonction de debug permettant d'afficher le tableau
