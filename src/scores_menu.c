...
void scores_menu(SDL_Renderer *renderer)
{
    ...
    renderScoreMenuHeader(renderer);
    renderPlayersTiles(renderer);
    renderPlayerInfos(renderer);
    while(!stop)
    {
       SDL_WaitEvent(&event);
       switch(event.type)
...
void renderScoreMenuHeader(SDL_Renderer *renderer)
{
    SDL_Texture *header;
    SDL_Rect font_rect;
...
void renderPlayersTiles(SDL_Renderer *renderer)
{
...
void renderPlayerInfos(SDL_Renderer *renderer)
{
    int size = nbJoueur();
    Joueur players[size];
    char name[size][20];
